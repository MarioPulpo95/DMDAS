...
#define dirPin 13
#define enablePin 3
#define startMotor 10
#define finecorsa 200 //da cambiare con la durata della corsa in mm
#define spostamento 1 // 1 mm per impulso
...
void receive_pulse(){
   ...
   if(rising){
      ...
      long impulsi = decodifica(dur1,dur2); //conta il numero di unità da 20ms ricevute dall'impulso del plc
...
void muovi_motore(long stepmot){
  ...
  if(directionState == HIGH){
      ...
      delayMicroseconds(2);
      int del = 800; //regola la velocità del motore, se è troppo piccolo non fa in tempo a muoversi
...
char cmd=Serial.read();
  switch(cmd){
      ...
      case 'b': //comunicami la posizione in openloop 
      Serial.println(GlobalPositionAttuatore);
      break;

      case 'c': //comunicami lo stato del motore (1 running , 0 Stop)
