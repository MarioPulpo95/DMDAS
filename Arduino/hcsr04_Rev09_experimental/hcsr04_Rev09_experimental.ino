...
void setup() {
  ...
  pinMode(startMotorPin, INPUT_PULLUP);

  //-----------Serial
  Serial.begin(38400);
  //attachInterrupt(digitalPinToInterrupt(pulseport), receive_pulse, CHANGE);
  attachInterrupt(digitalPinToInterrupt(pulseport), risingInterrupt, RISING);
  Serial.println('1');
...
void loop() {
  ...
  cmd_parser();
    ...
    if (readyToComm && !digitalRead(startMotorPin)) {
      ...
      Serial.println("muovo il motore");
...
void receive_pulse() {
  ...
  if (rising) {
    ...
    if (primaComm) {
      ...
      readyToComm = 1;
      Serial.println("comunicazione terminata");
      ...
      primaComm = 1;
      Serial.println("prima comm terminata");
    }
  }
}
///*---------------------------------------------------------------------*/
...
void risingInterrupt(){
  ...
  detachInterrupt(digitalPinToInterrupt(pulseport));
  Serial.println("salgo");
  
  attachInterrupt(digitalPinToInterrupt(pulseport), fallingInterrupt, FALLING);
  ...
  switch (cmd) {
    ...
    case 'd': //comunicami il dato letto dal PLC
     //Serial.println('1');
      ...
      else Serial.println("-99        timeout: controlla la connessione con il PLC");//
      timeout = 0;
      break;

    case 'm':
    
      impulsi = Serial.read()-'0';
      Serial.println(impulsi);
      readyToComm = 1;
...
void SetupClock() {
  ...
  if (clockgen.begin() != ERROR_NONE)
  {
    ...
    Serial.println("-88          Ooops, no Si5351 detected ... Check your wiring or I2C ADDR!");
    while (1);
  }
